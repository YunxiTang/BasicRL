...
class Actor(nn.Module):
    ...
    def forward(self, x, deterministic=False, with_logprob=True):
        ...
        if deterministic:  
            # use deterministic policy for evaluating
            a = mean
        else:
...
if __name__ == '__main__':
    ...
    env_evaluate = gym.make(env_platforms[idx], new_step_api=True)  # When evaluating the policy, we need to rebuild an environment
    number = 1
    seed = 0
    # Set random seed
    env.seed(seed)
    ...
    print("env={}".format(env_platforms[idx]))
    ...
    # Build a tensorboard
    writer = SummaryWriter(log_dir='runs/SAC/SAC_env_{}_number_{}_seed_{}'.format(env_platforms[idx], number, seed))

    max_train_steps = 3e6  # Maximum number of training steps
    random_steps = 1e3  # Take the random actions in the beginning for the better exploration
    ...
    while total_steps < max_train_steps:
        ...
        while not done:
            ...
            s_, r, done, _, _ = env.step(a)
            # r = reward_adapter(r, idx)  # Adjust rewards for better performance
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                # writer.add_scalar('step_rewards_{}'.format(env_platforms[idx]), evaluate_reward, global_step=total_steps)
                # Save the rewards
                # if evaluate_num % 10 == 0:
                    # np.save('./data_train/SAC_env_{}_number_{}_seed_{}.npy'.format(env_platforms[idx], number, seed), np.array(evaluate_rewards))
